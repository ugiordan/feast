...
Examples:
  ...
  # Scan multiple directories (intelligent discovery mode)
  %(prog)s config/ infra/operator/config/ --fail-on HIGH
  ...
  %(prog)s /path/to/repo --fail-on HIGH --include-attack-scenarios
    ...
    parser.add_argument('paths', nargs='+', help='Path(s) to directories to scan (supports multiple directories)')
    parser.add_argument(
        '--fail-on',
    ...
    if len(args.paths) == 1:
        print(f"Scanning directory: {args.paths[0]}")
    else:
        print(f"Scanning {len(args.paths)} directories:")
    ...
    # Load YAML files from all discovered directories
    for path in args.paths:
        print(f"Loading YAML files from: {path}")
        analyzer.load_yaml_files(path)
